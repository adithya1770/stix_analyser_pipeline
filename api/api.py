...
def calculate_risk(row):
    ...
    cred = float(row["credibility_score"]) / 100
    anom = int(row["anomaly_flag"])

    score = (0.6 * ml + 0.2 * cred + 0.2 * (1 - anom)) * 100
    score = round(score, 2)
...
async def analyze(file: UploadFile = File(...)):
    ...
    df["anomaly_flag"] = [1 if p == -1 else 0 for p in preds]

    df["credibility_score"] = df["confidence"]
